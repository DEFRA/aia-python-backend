...
logger = get_logger("app.agent_worker.main")
...
async def lifespan(_: FastAPI) -> AsyncGenerator[None, None]:
    ...
    logger.info("Agent worker process started")
    yield
    if _worker_task:
        _worker_task.cancel()
        try:
            ...
            pass
    logger.info("Agent worker process stopped")


app = FastAPI(title="AIA Agent Worker", version="1.0.0", lifespan=lifespan)
...
if __name__ == "__main__":
    try:
        ...
        logger.critical("Fatal agent worker error: %s", exc)
        sys.exit(1)
