from app.orchestrator.contracts import AgentResult, AssessmentRow, PolicyDocResult, Summary
...
def test_classify_risk_high_when_red_present():
    gen = MarkdownReportGenerator()
    assert gen._classify_risk(59) == "High Risk"


def test_classify_risk_medium_when_two_amber():
    gen = MarkdownReportGenerator()
    assert gen._classify_risk(75) == "Medium Risk"


def test_classify_risk_low_when_no_issues():
    gen = MarkdownReportGenerator()
    assert gen._classify_risk(90) == "Low Risk"
