"""Enterprise architecture specialist agent for document-based assessment."""
...
from src.agents.prompts.ea import SYSTEM_PROMPT, USER_PROMPT_TEMPLATE
from src.agents.schemas import AgentResult, AssessmentRow, FinalSummary, LLMResponseMeta
from src.config import EAAgentConfig
...
class EAAgent:
    """Async Claude agent that assesses a document against an EA checklist.
    ...
    def __init__(
        ...
        self.agent_config: EAAgentConfig = agent_config
    ...
    async def assess(
        ...
        """Run an enterprise architecture assessment of a document against a checklist.
        ...
        try:
            ...
            logger.error("Claude API error during EA assessment: %s", exc)
            raise
        ...
        try:
            ...
            ea_block: dict[str, object] = payload["EA"]  # type: ignore[assignment]
            assessments: list[AssessmentRow] = [
                AssessmentRow.model_validate(row)
                for row in ea_block["Assessments"]  # type: ignore[attr-defined]
            ]
            final_summary: FinalSummary | None = (
                FinalSummary.model_validate(ea_block["Final_Summary"])
                if "Final_Summary" in ea_block
                else None
            )
        ...
        logger.info(
            "EA assessment complete: %d questions, %d input / %d output tokens",
            len(assessments),
            meta.input_tokens,
            meta.output_tokens,
        )
