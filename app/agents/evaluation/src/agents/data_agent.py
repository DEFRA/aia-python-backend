"""Data governance specialist agent for document-based assessment."""
...
from src.agents.prompts.data import SYSTEM_PROMPT, USER_PROMPT_TEMPLATE
from src.agents.schemas import AgentResult, AssessmentRow, FinalSummary, LLMResponseMeta
from src.config import DataAgentConfig
...
class DataAgent:
    """Async Claude agent that assesses a document against a data governance checklist.
    ...
    def __init__(
        ...
        self.agent_config: DataAgentConfig = agent_config
    ...
    async def assess(
        ...
        """Run a data governance assessment of a document against a checklist.
        ...
        try:
            ...
            logger.error("Claude API error during data assessment: %s", exc)
            raise
        ...
        try:
            ...
            data_block: dict[str, object] = payload["Data"]  # type: ignore[assignment]
            assessments: list[AssessmentRow] = [
                AssessmentRow.model_validate(row)
                for row in data_block["Assessments"]  # type: ignore[attr-defined]
            ]
            final_summary: FinalSummary | None = (
                FinalSummary.model_validate(data_block["Final_Summary"])
                if "Final_Summary" in data_block
                else None
            )
        ...
        logger.info(
            "Data assessment complete: %d questions, %d input / %d output tokens",
            len(assessments),
            meta.input_tokens,
            meta.output_tokens,
        )
