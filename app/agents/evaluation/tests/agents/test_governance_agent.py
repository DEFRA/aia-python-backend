...
_QUESTIONS: list[QuestionItem] = [
    QuestionItem(question="Is a ROPA maintained?", reference="G1.a"),
    QuestionItem(question="Are retention schedules documented?", reference="G2.b"),
]
...
def _make_governance_payload(
    ...
    *,
    ...
    """Build a valid Governance JSON payload echoing the supplied references."""
...
def _make_agent(client: MagicMock) -> GovernanceAgent:
    config: GovernanceAgentConfig = GovernanceAgentConfig(ANTHROPIC_API_KEY="test-key")
    return GovernanceAgent(client=client, agent_config=config)
...
@pytest.mark.asyncio
async def test_governance_agent_assess_returns_agent_result() -> None:
    """assess() should parse the Governance JSON payload into an AgentResult."""
    payload: dict[str, Any] = _make_governance_payload(_QUESTIONS)
    client: MagicMock = _make_mock_client(json.dumps(payload))
    agent: GovernanceAgent = _make_agent(client)
...
async def test_governance_agent_validates_reference_echo() -> None:
    ...
    actual model output.
    """
    payload: dict[str, Any] = _make_governance_payload(
        _QUESTIONS,
        overrides={"G1.a": "WRONG-REF"},
    )
    client: MagicMock = _make_mock_client(json.dumps(payload))
    agent: GovernanceAgent = _make_agent(client)
    ...
    assert result.assessments[1].Reference.text == "G2.b"
...
@pytest.mark.asyncio
async def test_governance_agent_raises_on_invalid_payload() -> None:
    """Unparseable JSON or a missing ``Governance`` top-level key must raise ValueError."""
    # Case 1: malformed JSON
    bad_client: MagicMock = _make_mock_client("Not JSON at all")
    agent: GovernanceAgent = _make_agent(bad_client)
    ...
    # Case 2: valid JSON but missing top-level ``Governance`` key
