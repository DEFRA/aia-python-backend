...
from app.common.postgres import get_db_pool
...
class TestUploadSuccess:
    @patch("app.upload.router.service.check_duplicate", new_callable=AsyncMock, return_value=False)
    @patch("app.upload.router.upload_file_to_s3", new_callable=AsyncMock)
    @patch("app.upload.router.service.insert_document", new_callable=AsyncMock, return_value=MOCK_DOC_ID)
    @patch("app.upload.router.uuid.uuid4", return_value=uuid.UUID(MOCK_DOC_ID))
...
class TestUploadDuplicate:
    @patch("app.upload.router.service.check_duplicate", new_callable=AsyncMock, return_value=True)
...
class TestFetchHistory:
    @patch(
        "app.upload.router.service.fetch_history",
...
class TestGetResult:
    @patch(
        "app.upload.router.service.fetch_result",
        new_callable=AsyncMock,
        return_value=None,
    )
