...
async def verify_auth(request: Request) -> dict:
    ...
    claimed_user_id = request.headers.get(config.user_id_header)
    
    if not claimed_user_id:
        logger.warning("Authentication failed: Missing %s header", config.user_id_header)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail=f"Missing {config.user_id_header} header"
        )
