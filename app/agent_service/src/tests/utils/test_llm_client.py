"""Tests for utils.llm_client.make_llm_client."""
...
def test_anthropic_provider_returns_async_anthropic() -> None:
    ...
    with patch("app.agent_service.src.utils.llm_client.LLMConfig") as mock_cfg_cls:
        mock_cfg_cls.return_value.provider = "anthropic"
        with patch("app.agent_service.src.utils.llm_client.anthropic.AsyncAnthropic") as mock_cls:
...
def test_bedrock_provider_returns_async_anthropic_bedrock() -> None:
    ...
    with patch("app.agent_service.src.utils.llm_client.LLMConfig") as mock_cfg_cls:
        ...
        with patch("app.agent_service.src.utils.llm_client.anthropic.AsyncAnthropicBedrock") as mock_cls:
...
def test_provider_read_from_env_var() -> None:
    ...
    import app.agent_service.src.utils.llm_client as llm_module

    with (
        patch.dict(os.environ, {"LLM_PROVIDER": "bedrock"}),
        patch("app.agent_service.src.utils.llm_client.anthropic.AsyncAnthropicBedrock") as mock_bedrock,
    ):
...
def test_default_provider_is_bedrock() -> None:
    ...
    import app.agent_service.src.config as cfg_mod
