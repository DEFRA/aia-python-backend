...
**Environment variables:** Create a `.env` file at the root by copying `.env.example`:
```bash
cp .env.example .env
```

**Secrets:** `.env.secrets` (root level). You need to create this, as it's excluded from version control.
...
- Start dependent services with Docker Compose (Localstack, Postgres)
- Set up environment variables for local development
- Load configuration from .env and .env.secrets
